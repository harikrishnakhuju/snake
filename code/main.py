...
class Main:
    def __init__(self):
        ...
        # game objects
        self.bg_rects = [pygame.Rect((col + int(row % 2 == 0)) * CELL_SIZE, row * CELL_SIZE,CELL_SIZE,CELL_SIZE) for col in range(0,COLS,2) for row in range(ROWS)]  # int(row % 2 == 0) this will give 1 and add to col + for even row this create offset.
    ...
    def run(self):
        while True:
            for event in pygame.event.get():
                ...
                if event.type == self.update_event and self.game_active:  # only after 200ms the snake move
                    self.snake.update()
